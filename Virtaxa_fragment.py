...
def seq2seq_nofaa(db_folder,result_folder,csv_faa_file):
    linux_file=[]
    cmd_run("touch %s/tmp/job_hmmer_decision.sh"%(result_folder))
    pc = pc_('%s/mcl/mcl_train.out'%db_folder)
...
def treeshrink(treefile,contig):
    ...
    cmd1 = 'python run_treeshrink.py  -t %s -o %s'%(treefile,out_folder)
    cmd_run(cmd1)
...
def dist(treefile):
    ...
    novel_dis = dis_dict[novel_seq]
    if novel_dis > max_dis:
        partner = if_partner(tree,novel_name)
        if partner == 1 and novel_dis > 2*mean_dis:
            print('have sibling node but have a 2 times more distance：%s'%novel_name)
            result = 'reject'
        else:
            result = 'keep'
    else:
